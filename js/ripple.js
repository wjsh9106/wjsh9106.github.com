...
;(function ( $, window, document, undefined ) {
	...
	var img_width = 0;
	var img_height = 0;
	var p_width = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
	var p_height = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
	$("#img_container").css("width", p_width);
	$("#img_container").css("height", p_height);
    ...
    Plugin.prototype = {
        init: function() {
			...
			img_width = $(this.element).width();
			img_height = $(this.element).height();
			if(browserRedirect() == "phone")
			{
				if(img_width > p_width)
					scale = p_width/img_width;
				else if(scale == 1 && img_height > p_height)
					scale = p_height/img_height;
			...
			else
			{
				if(img_height > p_height)
					scale = p_height/img_height;
				else if(scale == 1 && img_width > p_width)
					scale = p_width/img_width;
			...
			if(p_height > img_height)
			 $("#img_container").css("margin", (p_height - img_height) / 2+"px auto");
			
            var scope = {};
            scope.canvas		= document.createElement('canvas');
			$(this.element).after(scope.canvas);
			scope.ctx			= scope.canvas.getContext('2d');
			scope.width			= img_width;
			scope.height		= img_height;
